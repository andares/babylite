...
scripts/verify-docs.mjs       # 链接与路径完整性校验
```
...
skill 目录包要求 `SKILL.md` 位于 skill 目录根部，仓库根即 skill 目录。任选一个 skill 根目录（按优先级）：
...
使用 DeepSeek harness 时也可以走自带命令：

```text
/skill install github:andares/babylite
```
