...
source: https://doc.babylonjs.com/lite/01-getting-started/
section: Guides
---
...
[Getting Started with Babylon Lite](https://doc.babylonjs.com/lite/01-getting-started#getting-started-with-babylon-lite) [1\. Install](https://doc.babylonjs.com/lite/01-getting-started#1-install) [2\. The mental model (read this first)](https://doc.babylonjs.com/lite/01-getting-started#2-the-mental-model-read-this-first) [Plain data, not classes](https://doc.babylonjs.com/lite/01-getting-started#plain-data-not-classes) [The scene is the sole owner](https://doc.babylonjs.com/lite/01-getting-started#the-scene-is-the-sole-owner) [An explicit lifecycle](https://doc.babylonjs.com/lite/01-getting-started#an-explicit-lifecycle) [You pay only for what you touch](https://doc.babylonjs.com/lite/01-getting-started#you-pay-only-for-what-you-touch) [3\. Your first scene](https://doc.babylonjs.com/lite/01-getting-started#3-your-first-scene) [4\. Loading a model and an environment](https://doc.babylonjs.com/lite/01-getting-started#4-loading-a-model-and-an-environment) [5\. How to think about common tasks](https://doc.babylonjs.com/lite/01-getting-started#5-how-to-think-about-common-tasks) [6\. Next steps](https://doc.babylonjs.com/lite/01-getting-started#6-next-steps)
...
> New here? Read **[Welcome](https://doc.babylonjs.com/lite)** first for the big picture and the "which engine should I use?" decision tree. This page assumes you've decided to build with Lite.

> 🛝 **Prefer to try before you install?** The **[Lite Playground](https://liteplayground.babylonjs.com/)** runs every example on this page in your browser — no setup required. See the **[Playground guide](https://doc.babylonjs.com/lite/04-playground)** for a tour.
...
This is the BoomBox scene from the **[Welcome](https://doc.babylonjs.com/lite)** page — the same model, lights, and IBL, in a handful of lines.
...
- **Reusing geometry across many objects.** Lite does **not** implement the classic `InstancedMesh` API. Instead, draw many copies of one mesh in a single GPU call with **thin instances** — pass a flat `Float32Array` of per-instance matrices via `setThinInstances(mesh, data, count)` (and optional per-instance colors). This is the idiomatic, high-performance way to populate a scene with repeated props. See **[Thin Instances](https://doc.babylonjs.com/lite/architecture/12-thin-instances)**.
- **Parenting and hierarchies.** Build transform hierarchies with `createTransformNode()` and parent nodes to it; world matrices propagate lazily. `cloneTransformNode()` duplicates a node subtree. See **[Scene Hierarchy & Parenting](https://doc.babylonjs.com/lite/architecture/11-scene-hierarchy-parenting)**.
...
- 🛝 **[Lite Playground](https://doc.babylonjs.com/lite/04-playground)** — run and tweak these scenes in your browser, then save and share them as a link.
- 🔁 **[Porting Guide](https://doc.babylonjs.com/lite/03-porting-guide)** — a side-by-side API map for translating a Babylon.js scene to Lite.
- 📊 **[Feature Comparison](https://doc.babylonjs.com/lite/02-feature-comparison)** — exactly what Lite covers today, what's partial, and what's missing.
- 🧱 **[Architecture docs](https://doc.babylonjs.com/lite/architecture/00-overview)** — deep dives into every subsystem, ordered from most to least commonly needed.
- 🧪 **[Headless (Null Engine)](https://doc.babylonjs.com/lite/05-headless-null-engine)** — run a scene without a GPU for server-side physics and deterministic simulation.
