---
title: Animation
source: https://doc.babylonjs.com/lite/architecture/07-animation/
...
[Module: Animation](https://doc.babylonjs.com/lite/architecture/07-animation/#module-animation) [Purpose](https://doc.babylonjs.com/lite/architecture/07-animation/#purpose) [Public API Surface](https://doc.babylonjs.com/lite/architecture/07-animation/#public-api-surface) [Constants](https://doc.babylonjs.com/lite/architecture/07-animation/#constants) [Types](https://doc.babylonjs.com/lite/architecture/07-animation/#types) [AnimationGroup Interface](https://doc.babylonjs.com/lite/architecture/07-animation/#animationgroup-interface) [AnimationGroupMask](https://doc.babylonjs.com/lite/architecture/07-animation/#animationgroupmask) [Functions](https://doc.babylonjs.com/lite/architecture/07-animation/#functions) [Internal Architecture](https://doc.babylonjs.com/lite/architecture/07-animation/#internal-architecture) [Data Layout](https://doc.babylonjs.com/lite/architecture/07-animation/#data-layout) [Keyframe Search](https://doc.babylonjs.com/lite/architecture/07-animation/#keyframe-search) [Scratch Buffer: `_quat`](https://doc.babylonjs.com/lite/architecture/07-animation/#scratch-buffer-_quat) [Frame Timing Model](https://doc.babylonjs.com/lite/architecture/07-animation/#frame-timing-model) [AnimationManager Lifecycle](https://doc.babylonjs.com/lite/architecture/07-animation/#animationmanager-lifecycle) [Usage Examples](https://doc.babylonjs.com/lite/architecture/07-animation/#usage-examples) [AnimationGroup Creation](https://doc.babylonjs.com/lite/architecture/07-animation/#animationgroup-creation) [Property Binding](https://doc.babylonjs.com/lite/architecture/07-animation/#property-binding) [Manual Weight Mixing](https://doc.babylonjs.com/lite/architecture/07-animation/#manual-weight-mixing) [glTF Skeleton Weight Mixing](https://doc.babylonjs.com/lite/architecture/07-animation/#gltf-skeleton-weight-mixing) [Pipeline Configuration](https://doc.babylonjs.com/lite/architecture/07-animation/#pipeline-configuration) [Shader Logic](https://doc.babylonjs.com/lite/architecture/07-animation/#shader-logic) [State Machine / Lifecycle](https://doc.babylonjs.com/lite/architecture/07-animation/#state-machine--lifecycle) [Babylon.js Equivalence Map](https://doc.babylonjs.com/lite/architecture/07-animation/#babylonjs-equivalence-map) [Dependencies](https://doc.babylonjs.com/lite/architecture/07-animation/#dependencies) [Test Specification](https://doc.babylonjs.com/lite/architecture/07-animation/#test-specification) [File Manifest](https://doc.babylonjs.com/lite/architecture/07-animation/#file-manifest)

> Package path: `packages/babylon-lite/src/animation/`

## Purpose

Provides a zero-allocation keyframe animation system for glTF animations and user-authored property animations. Supports skeletal joint transforms, morph target weights, KHR\_animation\_pointer channels, and manual tracks bound to object property paths such as `position.x`, `position`, `rotationQuaternion`, or `scaling`. Each evaluated clip is wrapped in an `AnimationGroup` and can be driven by a standalone `AnimationManager`, by a scene render loop, or by explicit deterministic updates. Evaluation runs LINEAR, STEP, and CUBICSPLINE interpolation using pre-allocated scratch buffers, with no per-frame heap allocation.
...
| --- | --- |
| Babylon.js API<br>`new Animation(name, "position.x", frameRate, FLOAT, CYCLE)` | Babylon Lite<br>`createPropertyAnimationClip(name, [{ path: "position.x", frameRate, keys }])` |
| Babylon.js API<br>`animation.setKeys(keys)` | Babylon Lite<br>keys passed to `createPropertyAnimationClip()` |
| Babylon.js API<br>`scene.beginDirectAnimation(target, [anim], from, to, loop)` | Babylon Lite<br>`createPropertyAnimationGroup(manager, target, clip, { fromFrame: from, toFrame: to, loop })` |
| Babylon.js API<br>`AnimationGroup` | Babylon Lite<br>`AnimationGroup` interface |
| Babylon.js API<br>`AnimationGroup.play()` | Babylon Lite<br>`playAnimation(group)` |
| Babylon.js API<br>`AnimationGroup.pause()` | Babylon Lite<br>`pauseAnimation(group)` |
| Babylon.js API<br>`AnimationGroup.stop()` | Babylon Lite<br>`stopAnimation(group)` |
| Babylon.js API<br>`AnimationGroup.goToFrame(f)` | Babylon Lite<br>`goToFrame(group, f)` (uses `group.frameRate`) |
| Babylon.js API<br>`AnimationGroup.speedRatio` | Babylon Lite<br>`group.speedRatio` |
| Babylon.js API<br>`AnimationGroup.loopAnimation` | Babylon Lite<br>`group.loopAnimation` |
| Babylon.js API<br>`AnimationGroup.weight` / `setWeightForAllAnimatables()` | Babylon Lite<br>`setAnimationWeight(group, weight)`; call `enableAnimationBlending(manager)` for weighted glTF skeleton clips |
| Babylon.js API<br>`AnimationGroup.MakeAnimationAdditive(group, frame)` | Babylon Lite<br>`setAnimationAdditive(group, { referenceFrame: frame })` |
| Babylon.js API<br>`AnimationGroup.mask = new AnimationGroupMask(names, mode)` | Babylon Lite<br>`group.mask = createAnimationGroupMask(names, mode)` |
| Babylon.js API<br>`AnimationGroupMask` / `AnimationGroupMaskMode` / `mask.retainsTarget(name)` | Babylon Lite<br>`AnimationGroupMask` / `AnimationGroupMaskMode` / `animationGroupMaskRetainsTarget(mask, name)` |
| Babylon.js API<br>manual property weighted blending | Babylon Lite<br>`enablePropertyAnimationBlending(manager)` \+ `setAnimationWeight(group, weight)` |
| Babylon.js API<br>manual weight ramp / blending speed | Babylon Lite<br>`fadeAnimationWeight(manager, group, { to, durationMs })` |
| Babylon.js API<br>manual cross-fade | Babylon Lite<br>`crossFadeAnimationGroups(manager, from, to, { durationMs })` |
| Babylon.js API<br>`scene.animationGroups` | Babylon Lite<br>`scene.animationGroups` |
| Babylon.js API<br>`Animation.ANIMATIONTYPE_QUATERNION` | Babylon Lite<br>`PATH_ROTATION = 1` |
| Babylon.js API<br>`Animation.ANIMATIONTYPE_VECTOR3` | Babylon Lite<br>`PATH_TRANSLATION = 0`, `PATH_SCALE = 2` |
...
01. **LINEAR interpolation**: Verify vec3 lerp and quat slerp produce correct intermediate values
02. **STEP interpolation**: Verify output snaps to keyframe value without blending
03. **CUBICSPLINE interpolation**: Verify Hermite spline evaluation with tangents; verify quaternion normalization
04. **Binary search edge cases**: `t` before first key, after last key, exactly on a key, between keys
05. **AnimationGroup lifecycle**: play → tick → verify time advances; pause → tick → verify time frozen; stop → verify reset to 0
06. **goToFrame**: Verify `goToFrame(120)` sets time to `2.0` seconds, evaluates the pose immediately, and pauses
07. **Looping**: Verify time wraps correctly at duration boundary
08. **Speed ratio**: Verify `speedRatio = 2` doubles playback speed
09. **Morph weight upload**: Verify correct weights written to GPU buffer for PATH\_WEIGHTS channels
...
| File | Purpose |
| --- | --- |
| File<br>`types.ts` | Purpose<br>All animation data types, interpolation/path constants, GPU-attached data interfaces |
| File<br>`evaluate.ts` | Purpose<br>Keyframe interpolation engine (LINEAR, STEP, CUBICSPLINE); binary search; zero-allocation |
| File<br>`animation-group.ts` | Purpose<br>User-facing AnimationGroup factory; wraps AnimationController per clip |
| File<br>`animation-group-mask.ts` | Purpose<br>Opt-in include/exclude target-name mask (`AnimationGroupMask`); installs the controller's mask resolver on first use |
| File<br>`animation-manager.ts` | Purpose<br>Generic animation task scheduler; no 2D, 3D, glTF, or property-animation runtime imports |
| File<br>`animation-group-task.ts` | Purpose<br>AnimationGroup-to-AnimationTask adapter, owner tracking, and `getAnimationGroups()` lookup |
| File<br>`property-animation.ts` | Purpose<br>Manual property animation clip builder and property path binding |
| File<br>`weighted-pointer-mixer.ts` | Purpose<br>Optional manual property weight mixer plus fade/cross-fade scheduling |
| File<br>`weighted-gltf-mixer.ts` | Purpose<br>Optional glTF skeleton weighted/additive mixer with single skeleton upload per blended target |
