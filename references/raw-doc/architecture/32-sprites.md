---
title: Sprites
source: https://doc.babylonjs.com/lite/architecture/32-sprites/
...
[Module: Sprites](https://doc.babylonjs.com/lite/architecture/32-sprites/#module-sprites) [Purpose](https://doc.babylonjs.com/lite/architecture/32-sprites/#purpose) [Pillars (front and centre)](https://doc.babylonjs.com/lite/architecture/32-sprites/#pillars-front-and-centre) [Taxonomy — Two Sprite Families](https://doc.babylonjs.com/lite/architecture/32-sprites/#taxonomy--two-sprite-families) [Why anchored is no longer a family](https://doc.babylonjs.com/lite/architecture/32-sprites/#why-anchored-is-no-longer-a-family) [Why billboards remain a separate family](https://doc.babylonjs.com/lite/architecture/32-sprites/#why-billboards-remain-a-separate-family) [Modes deliberately not added](https://doc.babylonjs.com/lite/architecture/32-sprites/#modes-deliberately-not-added) [Resolution: One engine loop, two registerable kinds](https://doc.babylonjs.com/lite/architecture/32-sprites/#resolution-one-engine-loop-two-registerable-kinds) [Rejected alternatives](https://doc.babylonjs.com/lite/architecture/32-sprites/#rejected-alternatives) [The engine's registration list](https://doc.babylonjs.com/lite/architecture/32-sprites/#the-engines-registration-list) [The `SpriteRenderer`](https://doc.babylonjs.com/lite/architecture/32-sprites/#the-spriterenderer) [Offscreen render target (render-to-texture)](https://doc.babylonjs.com/lite/architecture/32-sprites/#offscreen-render-target-render-to-texture) [Caller 1: pure-2D — no `SceneContext`](https://doc.babylonjs.com/lite/architecture/32-sprites/#caller-1-pure-2d--no-scenecontext) [Caller 2: scene-based — `registerScene` \+ `addToScene` \+ a separate HUD `SpriteRenderer`](https://doc.babylonjs.com/lite/architecture/32-sprites/#caller-2-scene-based--registerscene--addtoscene--a-separate-hud-spriterenderer) [Public API Surface](https://doc.babylonjs.com/lite/architecture/32-sprites/#public-api-surface) [Shared — Atlas and Frames](https://doc.babylonjs.com/lite/architecture/32-sprites/#shared--atlas-and-frames) [Family 1 — `Sprite2DLayer` (foundation)](https://doc.babylonjs.com/lite/architecture/32-sprites/#family-1--sprite2dlayer-foundation) [Sprite2D view utilities](https://doc.babylonjs.com/lite/architecture/32-sprites/#sprite2d-view-utilities) [Optional renderer-native Sprite2D Y-sort](https://doc.babylonjs.com/lite/architecture/32-sprites/#optional-renderer-native-sprite2d-y-sort) [Optional NPE → Sprite2D bridge integration](https://doc.babylonjs.com/lite/architecture/32-sprites/#optional-npe--sprite2d-bridge-integration) [Blend modes — tree-shakable descriptors](https://doc.babylonjs.com/lite/architecture/32-sprites/#blend-modes--tree-shakable-descriptors) [Per-layer custom fragment shaders](https://doc.babylonjs.com/lite/architecture/32-sprites/#per-layer-custom-fragment-shaders) [Opt-in per-sprite UV scroll (parallax)](https://doc.babylonjs.com/lite/architecture/32-sprites/#opt-in-per-sprite-uv-scroll-parallax) [Roadmap — `AnchorSource` opt-in 3D bridge for `Sprite2DLayer`](https://doc.babylonjs.com/lite/architecture/32-sprites/#roadmap--anchorsource-opt-in-3d-bridge-for-sprite2dlayer) [Family 2 — `*BillboardSpriteSystem`](https://doc.babylonjs.com/lite/architecture/32-sprites/#family-2--billboardspritesystem) [Picking](https://doc.babylonjs.com/lite/architecture/32-sprites/#picking) [Pure-2D usage — no scene](https://doc.babylonjs.com/lite/architecture/32-sprites/#pure-2d-usage--no-scene) [Internal Architecture](https://doc.babylonjs.com/lite/architecture/32-sprites/#internal-architecture) [Core Rule: No `if` Across Modes (still)](https://doc.babylonjs.com/lite/architecture/32-sprites/#core-rule-no-if-across-modes-still) [Per-Instance GPU Layout](https://doc.babylonjs.com/lite/architecture/32-sprites/#per-instance-gpu-layout) [Sprite2DLayer pure per-instance vertex buffer (52 B = 13 floats)](https://doc.babylonjs.com/lite/architecture/32-sprites/#sprite2dlayer-pure-per-instance-vertex-buffer-52-b--13-floats) [Sprite2DLayer depth-hosted extension (56 B = 14 floats)](https://doc.babylonjs.com/lite/architecture/32-sprites/#sprite2dlayer-depth-hosted-extension-56-b--14-floats) [Sprite2DLayer `uvScroll` extension (opt-in; +8 B = +2 floats)](https://doc.babylonjs.com/lite/architecture/32-sprites/#sprite2dlayer-uvscroll-extension-opt-in-8-b--2-floats) [Sprite2DLayer Layer UBO (48 B = 12 floats; std140-aligned)](https://doc.babylonjs.com/lite/architecture/32-sprites/#sprite2dlayer-layer-ubo-48-b--12-floats-std140-aligned) [Current BillboardSpriteSystem instance layout (64 B = 16 floats)](https://doc.babylonjs.com/lite/architecture/32-sprites/#current-billboardspritesystem-instance-layout-64-b--16-floats) [Future target BillboardSpriteSystem (96 B = 24 floats)](https://doc.babylonjs.com/lite/architecture/32-sprites/#future-target-billboardspritesystem-96-b--24-floats) [Vertexless Quad](https://doc.babylonjs.com/lite/architecture/32-sprites/#vertexless-quad) [CPU → GPU Sync (`sprite-gpu.ts`)](https://doc.babylonjs.com/lite/architecture/32-sprites/#cpu--gpu-sync-sprite-gputs) [Dirty / Version Tracking](https://doc.babylonjs.com/lite/architecture/32-sprites/#dirty--version-tracking) [Visibility (`visible: false`)](https://doc.babylonjs.com/lite/architecture/32-sprites/#visibility-visible-false) [Hook Registration Order](https://doc.babylonjs.com/lite/architecture/32-sprites/#hook-registration-order) [Pipeline Configuration](https://doc.babylonjs.com/lite/architecture/32-sprites/#pipeline-configuration) [Shared Across All Layers](https://doc.babylonjs.com/lite/architecture/32-sprites/#shared-across-all-layers) [Sprite2DLayer per-`depth` Pipeline State](https://doc.babylonjs.com/lite/architecture/32-sprites/#sprite2dlayer-per-depth-pipeline-state) [Bind Group Layouts](https://doc.babylonjs.com/lite/architecture/32-sprites/#bind-group-layouts) [Pipeline Cache](https://doc.babylonjs.com/lite/architecture/32-sprites/#pipeline-cache) [Shader Logic](https://doc.babylonjs.com/lite/architecture/32-sprites/#shader-logic) [Sprite2DLayer Vertex Shader (covers pure 2D AND anchored)](https://doc.babylonjs.com/lite/architecture/32-sprites/#sprite2dlayer-vertex-shader-covers-pure-2d-and-anchored) [Billboard Vertex Shaders](https://doc.babylonjs.com/lite/architecture/32-sprites/#billboard-vertex-shaders) [Facing (spherical) basis](https://doc.babylonjs.com/lite/architecture/32-sprites/#facing-spherical-basis) [Axis-Locked basis](https://doc.babylonjs.com/lite/architecture/32-sprites/#axis-locked-basis) [Shared vertex body](https://doc.babylonjs.com/lite/architecture/32-sprites/#shared-vertex-body) [Billboard Fragment Shader](https://doc.babylonjs.com/lite/architecture/32-sprites/#billboard-fragment-shader) [Sorting and Transparency](https://doc.babylonjs.com/lite/architecture/32-sprites/#sorting-and-transparency) [Picking](https://doc.babylonjs.com/lite/architecture/32-sprites/#picking-1) [`pickSprite2D` — CPU rectangle test](https://doc.babylonjs.com/lite/architecture/32-sprites/#picksprite2d--cpu-rectangle-test) [`pickBillboardSprite` — GPU pick into the shared mesh pass](https://doc.babylonjs.com/lite/architecture/32-sprites/#pickbillboardsprite--gpu-pick-into-the-shared-mesh-pass) [State Machine / Lifecycle](https://doc.babylonjs.com/lite/architecture/32-sprites/#state-machine--lifecycle) [Atlas + Layer Creation](https://doc.babylonjs.com/lite/architecture/32-sprites/#atlas--layer-creation) [Depth-Hosted Scene Admission](https://doc.babylonjs.com/lite/architecture/32-sprites/#depth-hosted-scene-admission) [Build (at `registerScene`)](https://doc.babylonjs.com/lite/architecture/32-sprites/#build-at-registerscene) [Per-Frame Render](https://doc.babylonjs.com/lite/architecture/32-sprites/#per-frame-render) [Disposal](https://doc.babylonjs.com/lite/architecture/32-sprites/#disposal) [Handles, Identity, and Parenting](https://doc.babylonjs.com/lite/architecture/32-sprites/#handles-identity-and-parenting) [Two-tier API design](https://doc.babylonjs.com/lite/architecture/32-sprites/#two-tier-api-design) [Stable IDs (`idToIndex` / `indexToId`)](https://doc.babylonjs.com/lite/architecture/32-sprites/#stable-ids-idtoindex--indextoid) [Roadmap observable handle field tables](https://doc.babylonjs.com/lite/architecture/32-sprites/#roadmap-observable-handle-field-tables) [`anchor` setter — anchored sprites are still Sprite2D handles](https://doc.babylonjs.com/lite/architecture/32-sprites/#anchor-setter--anchored-sprites-are-still-sprite2d-handles) [3D parenting (Billboard handles)](https://doc.babylonjs.com/lite/architecture/32-sprites/#3d-parenting-billboard-handles) [2D parenting (Sprite2D)](https://doc.babylonjs.com/lite/architecture/32-sprites/#2d-parenting-sprite2d) [Tree-shaking](https://doc.babylonjs.com/lite/architecture/32-sprites/#tree-shaking) [Future physics integration](https://doc.babylonjs.com/lite/architecture/32-sprites/#future-physics-integration) [Babylon.js Equivalence Map](https://doc.babylonjs.com/lite/architecture/32-sprites/#babylonjs-equivalence-map) [Roadmap Anchored Sizing — Common Porting Pitfalls](https://doc.babylonjs.com/lite/architecture/32-sprites/#roadmap-anchored-sizing--common-porting-pitfalls) [Dependencies](https://doc.babylonjs.com/lite/architecture/32-sprites/#dependencies) [Test Specification](https://doc.babylonjs.com/lite/architecture/32-sprites/#test-specification) [Unit (vitest)](https://doc.babylonjs.com/lite/architecture/32-sprites/#unit-vitest) [Visualization (Playwright)](https://doc.babylonjs.com/lite/architecture/32-sprites/#visualization-playwright) [Bundle Size Ceilings](https://doc.babylonjs.com/lite/architecture/32-sprites/#bundle-size-ceilings) [File Manifest](https://doc.babylonjs.com/lite/architecture/32-sprites/#file-manifest) [Sprite Frame Animation (Optional)](https://doc.babylonjs.com/lite/architecture/32-sprites/#sprite-frame-animation-optional) [Core Principles](https://doc.babylonjs.com/lite/architecture/32-sprites/#core-principles) [API](https://doc.babylonjs.com/lite/architecture/32-sprites/#api) [Usage Example](https://doc.babylonjs.com/lite/architecture/32-sprites/#usage-example) [Implementation Notes](https://doc.babylonjs.com/lite/architecture/32-sprites/#implementation-notes) [Public-API additions to `packages/babylon-lite/src/index.ts`](https://doc.babylonjs.com/lite/architecture/32-sprites/#public-api-additions-to-packagesbabylon-litesrcindexts)
...
| --- | --- | --- | --- | --- | --- |
| Family<br>`Sprite2DLayer` | Variants<br>1 (with optional `AnchorSource`) | Coordinate space<br>Pixels (layer-space; CPU-projected for anchored sprites) | Size unit<br>Pixels | Depth<br>Configurable per layer (composer-baked) | Drawn by<br>A `SpriteRenderer` registered on the engine (pure-2D, or HUD-on-3D registered after the scene), or — for `depth: "test" | "test-write"` layers — the scene's 3D pass |
| Family<br>`*BillboardSpriteSystem` | Variants<br>2: `Facing`, `AxisLocked` | Coordinate space<br>World | Size unit<br>World units | Depth<br>Read; write configurable | Drawn by<br>The scene's 3D pass (no pure-2D path) |
...
camera basis vectors **before** projection (`cameraRight * sizeWorld.x + cameraUp * sizeWorld.y`), which produces correct perspective
...
per-vertex `if (isBillboard) { compute world basis } else { compute pixel offset }` (violating the no-`if`-on-render-path rule), or a CPU "project
...
framebuffer: `framebufferX = viewport.x + screenX * viewport.width / targetWidth`, with the equivalent expression for Y. Pass the full render-target
...
maps, or typed arrays (see [Handles](https://doc.babylonjs.com/lite/architecture/32-sprites/#handles-identity-and-parenting)).
...
`particle/particle-sprite-2d.ts` is an opt-in consumer of the Sprite2D foundation. It creates one centered `depth: "none"` layer per NPE particle system and renders those layers through a caller-owned `SpriteRenderer`. The particle module owns coordinate, sprite-sheet, blend, synchronization, registration, and disposal policy; the sprite module continues to own layer storage and rendering. The complete bridge contract lives in [42-node-particle.md](https://doc.babylonjs.com/lite/architecture/42-node-particle#104-sprite2d-bridge-creation-and-mapping).

`particle/particle-sprite-2d-blend-modes.ts` is a separate particle-owned opt-in for exact NPE blend modes. It reuses ordinary `Sprite2DLayer`s and the existing custom-fragment API; mode 4 registers two equal-order layers consecutively so the renderer's stable sort preserves Multiply then Add. Generic Sprite2D does not interpret particle blend numbers, animate particle systems, or own multipass policy, and the baseline bridge does not import this advanced module. The exact API, shader formula, lifecycle, and tests live in [42-node-particle.md](https://doc.babylonjs.com/lite/architecture/42-node-particle#105-exact-sprite2d-blend-mode-bridge).
...
`createBillboardCustomShader` (see [Per-layer custom fragment shaders](https://doc.babylonjs.com/lite/architecture/32-sprites/#per-layer-custom-fragment-shaders)).
...
atlas-driven size changes should use `updateSprite2DIndex(layer, index, { frame, sizePx })` or `updateBillboardSpriteIndex(system, index, { frame, sizeWorld })` with an explicit size policy. For billboards, pixel
...
design is specified under [Picking](https://doc.babylonjs.com/lite/architecture/32-sprites/#picking) below.
...
`depth: "none"` uses 52 bytes / 13 floats and has no Z lane. `depth: "test" | "test-write"` uses 56 bytes / 14 floats and exposes slot \[13\]
...
| --- | --- | --- | --- | --- |
| Offset (bytes)<br>0..7 | Slot<br>\[0..1\] | Field<br>`positionPx` | Vertex attr<br>`@location(0)` f32×2 | Notes<br>layer-space pixels; for anchored sprites, written by the CPU sync hook |
| Offset (bytes)<br>8..15 | Slot<br>\[2..3\] | Field<br>`sizePx` | Vertex attr<br>`@location(1)` f32×2 | Notes<br>width/height in pixels; zeroed when `visible: false` (degenerate quad) |
| Offset (bytes)<br>16..23 | Slot<br>\[4..5\] | Field<br>`uvMin` | Vertex attr<br>`@location(2)` f32×2 | Notes<br>atlas UV min |
| Offset (bytes)<br>24..31 | Slot<br>\[6..7\] | Field<br>`uvMax` | Vertex attr<br>`@location(3)` f32×2 | Notes<br>atlas UV max |
| Offset (bytes)<br>32..35 | Slot<br>\[8\] | Field<br>`rotation` | Vertex attr<br>`@location(4)` f32 | Notes<br>radians; vertex shader takes `sin`/`cos` once per vertex |
| Offset (bytes)<br>36..51 | Slot<br>\[9..12\] | Field<br>`colorRGBA` | Vertex attr<br>`@location(5)` f32×4 | Notes<br>four float32 channels, matching Babylon.js SpriteRenderer color precision |
...
| Offset (bytes) | Slot | Field | Vertex attr | Notes |
| --- | --- | --- | --- | --- |
| Offset (bytes)<br>52..55 | Slot<br>\[13\] | Field<br>`z` | Vertex attr<br>`@location(6)` f32 | Notes<br>NDC depth (`0` = near, `1` = far), consumed by the scene pipeline |
...
| --- | --- | --- | --- | --- | --- |
| Layout<br>pure-2D + `uvScroll` | Stride<br>60 B / 15 fl | Slot<br>\[13..14\] | Field<br>`uvOffset` | Vertex attr<br>`@location(7)` f32×2 | Notes<br>`uvOffset.xy` at byte offset 52 |
| Layout<br>depth + `uvScroll` | Stride<br>64 B / 16 fl | Slot<br>\[14..15\] | Field<br>`uvOffset` | Vertex attr<br>`@location(7)` f32×2 | Notes<br>base 56 B + `uvOffset.xy` at offset 56 |
...
| --- | --- | --- |
| WGSL field<br>`viewPos` | Offset<br>0 | Notes<br>`vec2<f32>` — layer view's pan in pixels |
| WGSL field<br>`viewScale` | Offset<br>8 | Notes<br>`f32` — layer view's zoom |
| WGSL field<br>`viewRot` | Offset<br>12 | Notes<br>`f32` — layer view's rotation in radians |
| WGSL field<br>`screenSize` | Offset<br>16 | Notes<br>`vec2<f32>` — engine target size in pixels |
| WGSL field<br>`pivot` | Offset<br>24 | Notes<br>`vec2<f32>` — layer-wide rotation/scaling pivot in normalised sprite-local space |
| WGSL field<br>`opacityMul` | Offset<br>32 | Notes<br>`vec4<f32>` — pre-shaped per layer opacity. Straight-alpha: `(1, 1, 1, opacity)`. Premultiplied: `(o, o, o, o)`. CPU pre-shapes per blend mode so the shader is branch-free |
...
`@builtin(vertex_index)` for the four quad corners and `drawIndexed(6, system.count, 0, 0, 0)` for all active sprites.

| Offset (floats) | Field | Vertex format | Notes |
| --- | --- | --- | --- |
| Offset (floats)<br>0..2 | Field<br>`position` | Vertex format<br>`float32x3` | Notes<br>world-space anchor |
| Offset (floats)<br>3..4 | Field<br>`sizeWorld` | Vertex format<br>`float32x2` | Notes<br>zeroed when hidden; true size mirrored in `_savedSize` |
| Offset (floats)<br>5..6 | Field<br>`uvMin` | Vertex format<br>`float32x2` | Notes<br>frame UV minimum, swapped with max when flipped |
| Offset (floats)<br>7..8 | Field<br>`uvMax` | Vertex format<br>`float32x2` | Notes<br>frame UV maximum, swapped with min when flipped |
| Offset (floats)<br>9 | Field<br>`rotation` | Vertex format<br>`float32` | Notes<br>radians, applied in camera-facing local space |
| Offset (floats)<br>10..11 | Field<br>`pivot` | Vertex format<br>`float32x2` | Notes<br>normalized \[0,1\] pivot |
| Offset (floats)<br>12..15 | Field<br>`color` | Vertex format<br>`float32x4` | Notes<br>RGBA tint stored as four float32 channels |
...
| --- | --- | --- |
| Offset (floats)<br>0..2 | Field<br>`worldPos` | Notes<br>xyz — anchor position in world space |
| Offset (floats)<br>3 | Field<br>`_reserved` | Notes<br>0 (anchored layers use this slot for `depthBias`) |
| Offset (floats)<br>4..5 | Field<br>`_reserved` | Notes<br>(0,0) (anchored layers use these for `offsetPx`) |
| Offset (floats)<br>6..7 | Field<br>`sizeWorld` | Notes<br>width/height in world units |
| Offset (floats)<br>8..9 | Field<br>`pivot` | Notes<br>normalized \[0,1\] |
| Offset (floats)<br>10..11 | Field<br>`sinCos` | Notes<br>precomputed sin/cos of rotation |
| Offset (floats)<br>12..15 | Field<br>`uvRect` | Notes<br>uvMin.xy, uvMax.xy |
| Offset (floats)<br>16..19 | Field<br>`color` | Notes<br>RGBA tint |
| Offset (floats)<br>20..23 | Field<br>`flagsAndPad` | Notes<br>float-encoded `[flipX, flipY, _reserved, _reserved]` |
...
**Bounds center for engine-wide transparent sort.**`computeSpriteBoundsCenter(state, storage)` walks the first three floats of
...
| --- | --- | --- |
| Field<br>`_version` | Bumped by<br>All `add*` / `update*` / `remove*` / `set*Frame` / clip-advance helpers | Checked by<br>GPU sync |
| Field<br>`_gpuVersion` | Bumped by<br>GPU sync after upload | Checked by<br>— |
| Field<br>`_sortVersion` | Bumped by<br>Camera change (billboard families) or any 3D-position change | Checked by<br>Sort recomputation |
...
| Setting<br>Index buffer | Value<br>shared 6-index Uint16 quad index buffer (one per `SpriteRenderer` and one per depth-hosted renderable) |
| Setting<br>Cull mode | Value<br>`none` |
| Setting<br>Front face | Value<br>`ccw` |
| Setting<br>Color target | Value<br>engine swap-chain format |
| Setting<br>MSAA | Value<br>`SpriteRenderer` always records a direct swapchain pass with `sampleCount = 1`; depth-hosted Sprite2D layers inherit the scene render target's sample count inside the frame graph |
| Setting<br>Atlas sampler | Value<br>per-atlas (`linear` or `nearest`), `clamp-to-edge`, no mipmaps default |
...
| --- | --- | --- | --- | --- | --- | --- |
| Layer `depth`<br>`"none"` | Drawn via<br>A `SpriteRenderer` registered on the engine | Depth attachment<br>none | Depth compare<br>none | Depth write<br>`false` | Instance layout / Z<br>52 B / 13 floats; no slot \[13\] | Render order<br>engine registration order; layer order within the renderer |
| Layer `depth`<br>`"test"` | Drawn via<br>`addDepthHostedSpriteLayer` → `sprite-renderable.ts` (renderable `order = 200`) | Depth attachment<br>engine depth attachment | Depth compare<br>`greater-equal` | Depth write<br>`false` | Instance layout / Z<br>56 B / 14 floats; slot \[13\] consumed | Render order<br>scene transparent queue (after opaque meshes) |
| Layer `depth`<br>`"test-write"` | Drawn via<br>`addDepthHostedSpriteLayer` → `sprite-renderable.ts` (renderable `order = 100`) | Depth attachment<br>engine depth attachment | Depth compare<br>`greater-equal` | Depth write<br>`true` | Instance layout / Z<br>56 B / 14 floats; slot \[13\] consumed | Render order<br>direct-drawn after cached opaque meshes, before transparent |
...
| --- | --- | --- | --- |
| Binding<br>0 | Resource<br>`BillboardSystem` UBO | Shader stages<br>vertex + fragment | Notes<br>32 B: `opacityMul: vec4<f32>`, `axisAndCutoff: vec4<f32>` |
| Binding<br>1 | Resource<br>atlas texture | Shader stages<br>fragment | Notes<br>`texture_2d<f32>` from `system.atlas.texture.view` |
| Binding<br>2 | Resource<br>atlas sampler | Shader stages<br>fragment | Notes<br>filtering sampler from the atlas texture |
...
Transparent billboard shaders sample the atlas and return `sampleColor * tint * billboards.opacityMul`. Cutout billboard shaders use the same multiply but first
...
| --- | --- | --- | --- | --- | --- |
| Family / variant<br>Sprite2DLayer `depth: "none"` | Drawn through<br>a `SpriteRenderer` registered on the engine | Render slot<br>engine `_renderingContexts` (after the scene context) | Per-instance ordering<br>logical order, or stable ascending Y staging when `enableSprite2DYSort` is used | Blend<br>per-blend | Depth write<br>off |
| Family / variant<br>Sprite2DLayer `depth: "test"` blended | Drawn through<br>`addDepthHostedSpriteLayer` → `sprite-renderable.ts` (`order = 200`) | Render slot<br>scene transparent queue (after opaque meshes) | Per-instance ordering<br>consumed; per-instance depth test, no write | Blend<br>per-blend | Depth write<br>off |
| Family / variant<br>Sprite2DLayer `depth: "test"` cutout | Drawn through<br>`addDepthHostedSpriteLayer` → `sprite-renderable.ts` (`order = 200`) | Render slot<br>scene transparent queue (after opaque meshes) | Per-instance ordering<br>consumed; per-instance depth test, no write | Blend<br>none | Depth write<br>off |
| Family / variant<br>Sprite2DLayer `depth: "test-write"` | Drawn through<br>`addDepthHostedSpriteLayer` → `sprite-renderable.ts` (`order = 100`) | Render slot<br>direct draw after cached opaque meshes | Per-instance ordering<br>consumed; per-instance depth test + write | Blend<br>per-blend | Depth write<br>on |
| Family / variant<br>Billboard blended | Drawn through<br>`addFacingBillboardSystem` / `addAxisLockedBillboardSystem` (`order = 200`) | Render slot<br>scene transparent queue | Per-instance ordering<br>CPU-sorted far-to-near staging upload | Blend<br>per-blend | Depth write<br>off |
| Family / variant<br>Billboard cutout | Drawn through<br>`addFacingBillboardSystem` / `addAxisLockedBillboardSystem` (`order = 100`) | Render slot<br>direct draw after cached opaque meshes | Per-instance ordering<br>logical insertion order; GPU depth resolves | Blend<br>none | Depth write<br>on |
...
[18-picking.md](https://doc.babylonjs.com/lite/architecture/18-picking). `addFacingBillboardSystem` /
...
| Floats | Field | Notes |
| --- | --- | --- |
| Floats<br>0..2 | Field<br>`camRight` | Notes<br>row 0 of the column-major view matrix (`view[0,4,8]`) |
| Floats<br>3 | Field<br>`baseId` | Notes<br>`u32` — first pick ID assigned to this system |
| Floats<br>4..6 | Field<br>`camUp` | Notes<br>row 1 of the view matrix (`view[1,5,9]`) |
| Floats<br>7 | Field<br>`cutoff` | Notes<br>`f32` — alpha cutoff (cutout systems only; `0` otherwise) |
| Floats<br>8..10 | Field<br>`axis` | Notes<br>normalized lock axis (axis-locked only; zero for facing) |
| Floats<br>11 | Field<br>`_pad` | Notes<br>trailing pad |
...
| --- | --- | --- | --- |
| Tier<br> **Index API** | Functions<br>`addSprite2DIndex`, `updateSprite2DIndex`, `removeSprite2DIndex`, `clearSprite2DLayer`, `setSprite2DFrameIndex` (and `addBillboardSpriteIndex` etc. for billboards) | Returns<br>`number` (slot index) | Use for<br>Tile maps, scenery, particles, large fixed-layout HUDs. Maximum throughput, zero per-sprite GC. Indices are _not_ stable — `removeXIndex` swap-removes |
| Tier<br> **Handle API** | Functions<br>`addSprite2D`, `removeSprite2D`, `addBillboardSprite`, `removeBillboardSprite` and matching update helpers | Returns<br>`Sprite2DHandle` / `BillboardSpriteHandle` | Use for<br>Player characters, enemies, UI elements that move or will later be parented. Stable id, remove-safe update helpers |
...
| --- | --- | --- |
| Field<br>`position` | Slot floats it writes (per `SPRITE_2D_STRIDE = 20`)<br>`[off+0]` = x, `[off+1]` = y | Setter side-effects<br>Marks worldMatrix2D dirty; if parented, walker overrides next frame |
| Field<br>`sizePx` | Slot floats it writes (per `SPRITE_2D_STRIDE = 20`)<br>`[off+2]` = w·scale.x, `[off+3]` = h·scale.y (only when un-parented) | Setter side-effects<br>Marks slot dirty |
| Field<br>`pivot` | Slot floats it writes (per `SPRITE_2D_STRIDE = 20`)<br>`[off+4]`, `[off+5]` | Setter side-effects<br>— |
| Field<br>`scale` | Slot floats it writes (per `SPRITE_2D_STRIDE = 20`)<br>(none directly — scaled into sizePx) | Setter side-effects<br>Marks worldMatrix2D dirty; re-writes packed size |
| Field<br>`color` | Slot floats it writes (per `SPRITE_2D_STRIDE = 20`)<br>`[off+12..15]` | Setter side-effects<br>— |
| Field<br>`rotation` | Slot floats it writes (per `SPRITE_2D_STRIDE = 20`)<br>(via `updateSprite2DIndex` patch — sin/cos at `[off+6..7]`) | Setter side-effects<br>Marks worldMatrix2D dirty |
| Field<br>`frame` | Slot floats it writes (per `SPRITE_2D_STRIDE = 20`)<br>UV at `[off+8..11]` | Setter side-effects<br>Calls `setSprite2DFrameIndex` |
| Field<br>`visible` | Slot floats it writes (per `SPRITE_2D_STRIDE = 20`)<br>Toggles packed sizePx between value and 0 | Setter side-effects<br>Calls `writeSizePx` |
| Field<br>`layerZ` | Slot floats it writes (per `SPRITE_2D_STRIDE = 20`)<br>`[off+16]` | Setter side-effects<br>Clamped to `[0, 1]` |
| Field<br>`parent` | Slot floats it writes (per `SPRITE_2D_STRIDE = 20`)<br>(only `IParentable2D`; doesn't touch slot directly) | Setter side-effects<br>Adds/removes from `_parentedHandles`; installs walker on first parent |
| Field<br>`anchor` | Slot floats it writes (per `SPRITE_2D_STRIDE = 20`)<br>(none directly; CPU projection writes positionPx + layerZ each frame) | Setter side-effects<br>Setting `AnchorSource` adds to layer `_anchored` map; setting `null` removes |
...
| --- | --- | --- |
| Babylon.js<br>`SpriteManager` (2D usage) | Babylon Lite<br>`Sprite2DLayer` (any scene) | Notes<br>No separate 2D scene type |
| Babylon.js<br>`SpriteManager` (3D usage, world-sized) | Babylon Lite<br>`*BillboardSpriteSystem` | Notes<br>Always world-space, perspective-correct |
| Babylon.js<br>`SpritePackedManager` | Babylon Lite<br>Roadmap named-atlas wrapper + family factory | Notes<br>Current atlas path is numeric grid/load only |
| Babylon.js<br>`Sprite` | Babylon Lite<br>`*Init` interfaces + per-family index helpers | Notes<br>Current API returns mutable slot indices |
| Babylon.js<br>`sprite.cellIndex` / `cellRef` | Babylon Lite<br>`setSprite*FrameIndex(layer, idx, frame)` | Notes<br>Current `frame` is numeric |
| Babylon.js<br>`sprite.playAnimation(from, to, loop, delay, cb)` | Babylon Lite<br>Roadmap clip helpers | Notes<br>Named clips are not implemented in this branch |
| Babylon.js<br>`sprite.invertU` / `invertV` | Babylon Lite<br>`init.flipX` / `init.flipY` | Notes |
| Babylon.js<br>`sprite.angle` | Babylon Lite<br>`init.rotation` (radians) | Notes |
| Babylon.js<br>`sprite.position` | Babylon Lite<br>`init.positionPx` (Sprite2D) / `init.position` (Billboard) | Notes<br>World anchoring for Sprite2D is roadmap |
| Babylon.js<br>`sprite.size` / `width` / `height` | Babylon Lite<br>`init.sizePx` (Sprite2D) / `init.sizeWorld` (Billboard) | Notes<br>Type encodes pixel-space vs. world-space |
| Babylon.js<br>`sprite.color` | Babylon Lite<br>`init.color` / `update*({ color: [r,g,b,a] })` | Notes<br>Per-sprite tint |
| Babylon.js<br>`mesh.billboardMode = BILLBOARDMODE_ALL` | Babylon Lite<br>`createFacingBillboardSystem` | Notes<br>Explicit factory |
| Babylon.js<br>`mesh.billboardMode = BILLBOARDMODE_Y` | Babylon Lite<br>`createAxisLockedBillboardSystem(atlas, [0,1,0])` | Notes<br>World-Y is the yaw-locked special case |
| Babylon.js<br>`mesh.billboardMode = BILLBOARDMODE_X/Z` | Babylon Lite<br>`createAxisLockedBillboardSystem(atlas, [1,0,0])` | Notes<br>Same factory covers all lock axes |
| Babylon.js<br>`SpriteManager.disableDepthWrite` | Babylon Lite<br>`Sprite2DLayer.depth` (`"test"` / `"test-write"`) \+ `SpriteBlendMode` | Notes<br>Composer-baked per layer |
| Babylon.js<br>`sprite.blendMode` (ADD / MULTIPLY / etc.) | Babylon Lite<br>Importable `spriteBlend*` / `billboardBlend*` descriptor values | Notes<br>Tree-shakable; no string lookup table |
| Babylon.js<br>Custom `ShaderMaterial` on a sprite | Babylon Lite<br>`createSprite2DCustomShader` / `createBillboardCustomShader` | Notes<br>WGSL fragment body + `fx.time` / `fx.params` / extra textures |
| Babylon.js<br>Animated/scrolling texture (`uOffset`/`vOffset`) | Babylon Lite<br>`Sprite2DLayerOptions.uvScroll` \+ per-sprite `uvOffset` | Notes<br>Opt-in per-sprite UV offset (parallax) |
| Babylon.js<br>`AdvancedDynamicTexture` \+ `Image` | Babylon Lite<br>`Sprite2DLayer` overlay on a 3D `SceneContext` | Notes<br>Different scope — no GUI tree |
| Babylon.js<br>`scene.pickSprite(x, y)` | Babylon Lite<br>`pickSprite2D` (CPU) / `pickBillboardSprite` (GPU) | Notes<br>CPU rect test for layers; GPU pick into the shared mesh pass for billboards |
| Babylon.js<br>`SpriteMap` (tile maps) | Babylon Lite<br>Out of scope | Notes<br>Future module |
| Babylon.js<br>`SpriteManager``epsilon` arg | Babylon Lite<br> _no equivalent_ | Notes<br>Atlases must have transparent border / NPOT / padded sub-rects when bleed matters |
| Babylon.js<br>Quad VBO | Babylon Lite<br>Vertexless (`vertex_index`) | Notes<br>Eliminates the static quad buffer |
