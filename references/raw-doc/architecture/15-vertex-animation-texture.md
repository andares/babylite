---
title: Vertex Animation Texture
source: https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/
...
[Module: Vertex Animation Texture (VAT)](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#module-vertex-animation-texture-vat) [Purpose](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#purpose) [Public API Surface](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#public-api-surface) [Functions (`vat/vat-baker.ts`)](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#functions-vatvat-bakerts) [Types (`vat/vat-baker.ts`)](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#types-vatvat-bakerts) [Internal type (`animation/types.ts`, not exported — like `SkeletonData`)](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#internal-type-animationtypests-not-exported--like-skeletondata) [Mesh feature bits (`material/mesh-features.ts`)](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#mesh-feature-bits-materialmesh-featurests) [Internal Architecture](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#internal-architecture) [Baked texture format (identical to the live bone texture)](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#baked-texture-format-identical-to-the-live-bone-texture) [Preparation and upload](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#preparation-and-upload) [Baking (`bakeVat`)](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#baking-bakevat) [Settings UBO (32 bytes, `attachVat`)](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#settings-ubo-32-bytes-attachvat) [Per-instance params texture (`setInstances` / `setInstancesBlend`)](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#per-instance-params-texture-setinstances--setinstancesblend) [Pipeline Configuration](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#pipeline-configuration) [Group-1 vertex bindings (pushed by `pbrExt.bind`, in declaration order)](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#group-1-vertex-bindings-pushed-by-pbrextbind-in-declaration-order) [Vertex attributes / builtins](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#vertex-attributes--builtins) [Shadow casting (free)](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#shadow-casting-free) [Shader Logic (vertex `VW` slot, WGSL outline)](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#shader-logic-vertex-vw-slot-wgsl-outline) [State Machine / Lifecycle](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#state-machine--lifecycle) [Babylon.js Equivalence Map](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#babylonjs-equivalence-map) [Dependencies](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#dependencies) [Test Specification](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#test-specification) [File Manifest](https://doc.babylonjs.com/lite/architecture/15-vertex-animation-texture/#file-manifest)
...
| --- | --- | --- |
| binding<br>b | resource<br>`vatSampler` — `texture_2d<f32>` (unfilterable-float), vertex-visible | when<br>always (MSH\_VAT) |
| binding<br>b+1 | resource<br>`vat` — uniform buffer (settings UBO) | when<br>always |
| binding<br>b+2 | resource<br>`vatInstanceTex` — `texture_2d<f32>` (unfilterable-float) | when<br>`MSH_VAT && MSH_HAS_THIN_INSTANCES` (instanceTexture set) |
...
| Babylon.js | Lite VAT |
| --- | --- |
| Babylon.js<br>`VertexAnimationBaker.bakeVertexData(ranges)` | Lite VAT<br>`bakeVat(engine, mesh, groups)` |
| Babylon.js<br>`BakedVertexAnimationManager` (manager.time, setAnimationParameters) | Lite VAT<br>`VatHandle` (`update`, `play`) |
| Babylon.js<br>`mesh.bakedVertexAnimationManager` | Lite VAT<br>`mesh.vat` (`VatData`) |
| Babylon.js<br>`manager.texture` (VAT texture) | Lite VAT<br>`VatBakeResult.texture` / `VatData.texture` |
| Babylon.js<br>Per-instance via `bakedVertexAnimationSettingsInstanced` buffer | Lite VAT<br>`setInstances` / `setInstancesBlend` → `instanceTexture` (read by `instance_index`) |
