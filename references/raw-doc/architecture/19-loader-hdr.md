---
title: Loader HDR
source: https://doc.babylonjs.com/lite/architecture/19-loader-hdr/
...
[Module: Loader HDR](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#module-loader-hdr) [Purpose](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#purpose) [Public API Surface](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#public-api-surface) [Functions](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#functions) [Types](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#types) [Internal Architecture](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#internal-architecture) [Stage 1: RGBE Parsing — `parseRGBE()`](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#stage-1-rgbe-parsing--parsergbe) [Stage 2: Spherical Harmonics — `computeSHFromEquirect()`](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#stage-2-spherical-harmonics--computeshfromequirect) [Stage 3: Equirect → Cubemap — `equirectToCubemapGPU()`](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#stage-3-equirect--cubemap--equirecttocubemapgpu) [Stage 4: Cubemap Prefiltering — `prefilterCubemapGPU()`](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#stage-4-cubemap-prefiltering--prefiltercubemapgpu) [Stage 5: BRDF LUT — `generateBrdfLut()`](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#stage-5-brdf-lut--generatebrdflut) [Full Pipeline Orchestration — `loadHdrEnvironment()`](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#full-pipeline-orchestration--loadhdrenvironment) [Pipeline Configuration](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#pipeline-configuration) [Compute Pipeline: Equirect → Cubemap](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#compute-pipeline-equirect--cubemap) [Compute Pipeline: Cubemap Prefilter](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#compute-pipeline-cubemap-prefilter) [Compute Pipeline: BRDF LUT](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#compute-pipeline-brdf-lut) [Shader Logic](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#shader-logic) [State Machine / Lifecycle](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#state-machine--lifecycle) [Babylon.js Equivalence Map](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#babylonjs-equivalence-map) [Dependencies](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#dependencies) [Test Specification](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#test-specification) [File Manifest](https://doc.babylonjs.com/lite/architecture/19-loader-hdr/#file-manifest)
...
export function computeSHFromEquirect(data: Float32Array, width: number, height: number): Float32Array; // 27 floats: SphericalPolynomial coefficients
```
...
- Face corner lookup: 24 pre-computed `vec3<f32>` corners (4 per face), matching BJS `panoramaToCubemap.ts` layout:
  ...
  - Layer 0: FACE\_RIGHT, Layer 1: FACE\_LEFT, Layer 2: FACE\_UP, Layer 3: FACE\_DOWN, Layer 4: FACE\_FRONT, Layer 5: FACE\_BACK
...
- **D\_GGX**: `D = a² / (π * ((N·H)²(a²-1)+1)²)` — GGX normal distribution
...
| --- | --- |
| Babylon.js<br>`HDRCubeTexture` | Babylon Lite<br>`loadHdrEnvironment()` |
| Babylon.js<br>`HDRTools.RGBE_ReadPixels` | Babylon Lite<br>`parseRGBE()` → `decodeScanline()` |
| Babylon.js<br>`CubeMapToSphericalPolynomialTools` | Babylon Lite<br>`computeSHFromEquirect()` |
| Babylon.js<br>`panoramaToCubemap.ts` face corners | Babylon Lite<br>`CORNERS` const array in WGSL |
| Babylon.js<br>`HDRFiltering` (importance-sampled GGX) | Babylon Lite<br>`prefilterCubemapGPU()` compute shader |
| Babylon.js<br>`BRDFTextureTools.GetBRDFTexture` | Babylon Lite<br>`generateBrdfLut()` compute shader |
| Babylon.js<br>`EnvironmentTextureTools` | Babylon Lite<br>`assembleEnvironmentTextures()` |
...
| File | Purpose |
| --- | --- |
| File<br>`load-hdr.ts` | Purpose<br>Public API: orchestrates full HDR→IBL pipeline, sets up scene environment and deferred builders |
| File<br>`hdr-parser.ts` | Purpose<br>CPU-side RGBE parsing and spherical harmonics extraction from equirectangular panorama |
| File<br>`hdr-ibl-pipeline.ts` | Purpose<br>GPU compute shaders: equirect→cubemap, importance-sampled GGX prefiltering, BRDF LUT generation |
