---
title: Text
source: https://doc.babylonjs.com/lite/architecture/33-text/
...
[Module: Text](https://doc.babylonjs.com/lite/architecture/33-text/#module-text) [Purpose](https://doc.babylonjs.com/lite/architecture/33-text/#purpose) [Public API Surface](https://doc.babylonjs.com/lite/architecture/33-text/#public-api-surface) [Tier 1 — `GlyphStorage` (longest-lived)](https://doc.babylonjs.com/lite/architecture/33-text/#tier-1--glyphstorage-longest-lived) [Tier 2 — `TextData`](https://doc.babylonjs.com/lite/architecture/33-text/#tier-2--textdata) [Tier 3a — `TextRenderable` (3D, attached to a scene)](https://doc.babylonjs.com/lite/architecture/33-text/#tier-3a--textrenderable-3d-attached-to-a-scene) [Tier 3b — `TextRenderer` \+ `TextLayer` (standalone 2D)](https://doc.babylonjs.com/lite/architecture/33-text/#tier-3b--textrenderer--textlayer-standalone-2d) [Tier 4 — Default helpers (depend on `text-shaper`)](https://doc.babylonjs.com/lite/architecture/33-text/#tier-4--default-helpers-depend-on-text-shaper) [Minimal example — `createDefaultTextData` \+ `TextRenderer`](https://doc.babylonjs.com/lite/architecture/33-text/#minimal-example--createdefaulttextdata--textrenderer) [Implementation](https://doc.babylonjs.com/lite/architecture/33-text/#implementation) [CPU data layout](https://doc.babylonjs.com/lite/architecture/33-text/#cpu-data-layout) [Per-instance layout (5 × `vec4`, 80 bytes)](https://doc.babylonjs.com/lite/architecture/33-text/#per-instance-layout-5--vec4-80-bytes) [Slot allocator (fast incremental updates)](https://doc.babylonjs.com/lite/architecture/33-text/#slot-allocator-fast-incremental-updates) [Dirty range + version-based GPU upload](https://doc.babylonjs.com/lite/architecture/33-text/#dirty-range--version-based-gpu-upload) [Spatial-band index (internal to `glyph-storage.ts`)](https://doc.babylonjs.com/lite/architecture/33-text/#spatial-band-index-internal-to-glyph-storagets) [Pipeline (`_gpu/text-pipeline.ts`)](https://doc.babylonjs.com/lite/architecture/33-text/#pipeline-_gputext-pipelinets) [WGSL outline](https://doc.babylonjs.com/lite/architecture/33-text/#wgsl-outline) [`TextRenderable` (3D) per-frame](https://doc.babylonjs.com/lite/architecture/33-text/#textrenderable-3d-per-frame) [`TextRenderer` (2D) per-frame](https://doc.babylonjs.com/lite/architecture/33-text/#textrenderer-2d-per-frame) [Default layout (`layout.ts`)](https://doc.babylonjs.com/lite/architecture/33-text/#default-layout-layoutts) [Default curve extraction (`glyph-extraction.ts`)](https://doc.babylonjs.com/lite/architecture/33-text/#default-curve-extraction-glyph-extractionts) [Dependencies](https://doc.babylonjs.com/lite/architecture/33-text/#dependencies) [Reference scenes](https://doc.babylonjs.com/lite/architecture/33-text/#reference-scenes) [Test specification](https://doc.babylonjs.com/lite/architecture/33-text/#test-specification) [File inventory](https://doc.babylonjs.com/lite/architecture/33-text/#file-inventory)
...
A `GlyphStorage` is an opaque bundle of `(curveSetId → glyph outlines + packed atlas)`. Each `CurveSetId` (a string, typically the font family name) maps to
...
| --- | --- | --- |\
| Field<br>`slugBounds` | Floats<br>4 | Contents<br>`(xMin, yMin, xMax, yMax)` in font units — the quad's extent |\
| Field<br>`slugAnchor` | Floats<br>4 | Contents<br>`(xPx, yPx, 1/pixelsPerFontUnit, deadSentinel)` — pixel origin + scale; `.w = 1` marks the slot dead |\
| Field<br>`slugAtlas` | Floats<br>4 | Contents<br>`(glyphLocX, glyphLocY, bandMaxX, bandMaxY)` — band texture lookup base + max band indices |\
| Field<br>`slugBand` | Floats<br>4 | Contents<br>`(bandScaleX, bandScaleY, bandOffsetX, bandOffsetY)` — derived from glyph bounds + band counts |\
| Field<br>`slugColor` | Floats<br>4 | Contents<br>linear RGBA per glyph (falls back to run `defaultColor`, then white) |\
\
...
`uploadedVersion !== atlas.version`. It returns `{ rebuilt, gpu }`; `rebuilt = true` (texture object identity changed) is the signal for the renderer to drop\
...
\
The renderer's pipeline is cached at `(swapchain format, sampleCount=1, no depth, depthWrite=false, flipY=false)`; depth-less means depth-hosted text-on-3D\
...
| File | Responsibility |\
| --- | --- |\
| File<br>`src/text/font.ts` | Responsibility<br>`Font` (branded type) + `loadFont` \+ `createFontFromBuffer`; text-shaper boundary. |\
| File<br>`src/text/glyph-extraction.ts` | Responsibility<br>`extractGlyphCurves` \+ `cubicToQuadratics`; default text-shaper-backed glyph path extraction. Outline value types live in `glyph-storage.ts`. |\
| File<br>`src/text/glyph-storage.ts` | Responsibility<br>`GlyphStorage` (branded) + `CurveSetId` \+ outline value types (`QuadCurve`, `GlyphBounds`, `GlyphCurves`) \+ the supporting `SharedAtlas` / `AtlasSlot` / `GlyphBands` types and their packers (`packAppendGlyph`, `buildGlyphBands`). Public API: `createGlyphStorage` / `updateGlyphStorage` / `disposeGlyphStorage`. |\
| File<br>`src/text/layout.ts` | Responsibility<br>`TextLayoutOptions` \+ `layoutText` — default LTR + word-wrap + align layout via `text-shaper.shape`. |\
| File<br>`src/text/text-data.ts` | Responsibility<br>`TextData` (branded) + `GlyphRun` \+ `PlacedGlyph` \+ `TextDataUpdate` \+ slot-allocator types. Public API: `createTextData` / `updateTextData` / `disposeTextData` and the per-instance allocator (addRun / removeRun / replaceRun / reset+compaction). |\
| File<br>`src/text/default-text-data.ts` | Responsibility<br>`DefaultTextData` (branded) + `createDefaultTextData` / `updateDefaultTextData` / `disposeDefaultTextData`; convenience layer composing layout + curve extraction + a private `GlyphStorage`. |\
| File<br>`src/text/text-renderable.ts` | Responsibility<br>`TextRenderable` \+ `createTextRenderable` / `addTextRenderable` / `disposeTextRenderable`; 3D `Renderable` implementation. |\
| File<br>`src/text/text-renderer.ts` | Responsibility<br>`TextLayer` (2D pixel-space placement record) + `TextRenderer` (standalone `RenderingContext`) \+ their factories and the swapchain draw pass. |\
| File<br>`src/text/_gpu/text-textures.ts` | Responsibility<br>`ensureSharedAtlasGpu`; lazy `rgba32float` texture create + version-gated upload + capacity grow. (Atlas teardown is inlined in `disposeGlyphStorage` to avoid a circular import.) |\
| File<br>`src/text/_gpu/text-pipeline.ts` | Responsibility<br>`getOrCreateTextPipeline` / `clearTextPipelineCache`; per-device bind group layout + WGSL modules + per-target-key pipeline cache. |\
| File<br>`src/text/shaders/slug.vert.wgsl` | Responsibility<br>Vertex stage: dead-slot collapse + Slug dilation + MVP transform. |\
| File<br>`src/text/shaders/slug.frag.wgsl` | Responsibility<br>Fragment stage: per-band quadratic root solve + signed coverage accumulation. |\
\
