---
title: Post Process
source: https://doc.babylonjs.com/lite/architecture/29-post-process/
...
[Module: Frame-Graph Post-Process](https://doc.babylonjs.com/lite/architecture/29-post-process/#module-frame-graph-post-process) [Purpose](https://doc.babylonjs.com/lite/architecture/29-post-process/#purpose) [Public API Surface (types, functions, constants — full signatures)](https://doc.babylonjs.com/lite/architecture/29-post-process/#public-api-surface-types-functions-constants--full-signatures) [Internal Architecture (data structures, memory layouts)](https://doc.babylonjs.com/lite/architecture/29-post-process/#internal-architecture-data-structures-memory-layouts) [Pipeline Configuration (vertex/fragment stages, bind groups, depth/stencil)](https://doc.babylonjs.com/lite/architecture/29-post-process/#pipeline-configuration-vertexfragment-stages-bind-groups-depthstencil) [Shader Logic (WGSL outline or pseudocode with exact math)](https://doc.babylonjs.com/lite/architecture/29-post-process/#shader-logic-wgsl-outline-or-pseudocode-with-exact-math) [State Machine / Lifecycle](https://doc.babylonjs.com/lite/architecture/29-post-process/#state-machine--lifecycle) [Babylon.js Equivalence Map](https://doc.babylonjs.com/lite/architecture/29-post-process/#babylonjs-equivalence-map) [Dependencies](https://doc.babylonjs.com/lite/architecture/29-post-process/#dependencies) [Test Specification](https://doc.babylonjs.com/lite/architecture/29-post-process/#test-specification) [File Manifest](https://doc.babylonjs.com/lite/architecture/29-post-process/#file-manifest)
...
| --- | --- | --- |
| Binding<br>`0` | Resource<br>source sampler (`nearest` or `linear`) | Visibility<br>fragment |
| Binding<br>`1` | Resource<br>source color texture (`texture_2d<f32>`) | Visibility<br>fragment |
| Binding<br>`2+` | Resource<br>optional extra textures (`texture_2d<f32>`) | Visibility<br>fragment |
| Binding<br>`2 + extraTextures.length` or `_shader.uniformBinding` | Resource<br>optional uniform buffer | Visibility<br>fragment |
...
01. Caller creates a producer whose target owns a single-sample color texture. The producer can be a scene `RenderTask`, an `EffectRenderTask`, or any custom task.
02. Scene pipelines use the scene's frame graph as before. Scene-less pipelines create `const ctx = createFrameGraphContext(engine, { update })`; the update callback runs once per frame before graph execution and is the right place to update effect uniforms.
03. Caller creates the post-process task with `sourceTexture` set to the producer target. If `targetTexture` is omitted, the task creates an internal color-only target using the source descriptor's format, sample count, and size. If the post-process writes the canvas, use a target with `resolveToSwapchain: true`.
04. Caller schedules tasks in order with `addTask(sceneOrFrameGraph, producer)`, then `addTask(sceneOrFrameGraph, postProcess)`, then any consumer/output task.
05. Scene-less graphs are started with `registerFrameGraphContext(ctx)`, which builds `ctx.frameGraph` and registers the rendering context with the engine. Later canvas resizes rebuild the graph through the context `_resize` hook.
06. `FrameGraph.build()` calls the producer `record()` first, allocating source texture resources.
07. Post-process `record()` validates the source color texture, allocates/rebuilds the output target, creates the pipeline, bind group, and optional uniform buffer, initializes uniforms, and records no separate `Pass` because the task executes directly.
08. If a caller mutates exposed post-process parameters after recording, they call `updateUniforms()` to rewrite the task UBO before the next draw.
09. Each frame, `execute()` patches swapchain attachments if needed, applies viewport/scissor if requested, binds pipeline/group, and draws one full-screen triangle.
...
| --- | --- |
| Babylon.js<br>`FrameGraphPostProcessTask` | Babylon Lite<br>`PostProcessTask` plain task record |
| Babylon.js<br>`sourceTexture` handle | Babylon Lite<br>`RenderTarget` source with sampled color attachment |
| Babylon.js<br>`targetTexture` handle | Babylon Lite<br>optional `RenderTarget` output |
| Babylon.js<br>auto-created output texture | Babylon Lite<br>internal `RenderTarget` copied from source descriptor |
| Babylon.js<br>`sourceSamplingMode` | Babylon Lite<br>`"nearest"` / `"linear"` sampler selection |
| Babylon.js<br>`alphaMode` | Babylon Lite<br>numeric modes `0`, `1`, `2`, `7` mapped to WebGPU blend state |
| Babylon.js<br>`viewport` | Babylon Lite<br>normalized viewport/scissor on the output pass |
| Babylon.js<br>`ThinPostProcess` | Babylon Lite<br>not present; shader config is direct WGSL |
| Babylon.js<br>`FrameGraphBlackAndWhiteTask` | Babylon Lite<br>`createBlackAndWhitePostProcessTask` |
| Babylon.js<br>`FrameGraphAnaglyphTask` / `ThinAnaglyphPostProcess` | Babylon Lite<br>`createAnaglyphPostProcessTask` |
| Babylon.js<br>`FrameGraphBlurTask` / `ThinBlurPostProcess` | Babylon Lite<br>`createBlurPostProcessTask` |
| Babylon.js<br>`FrameGraphExtractHighlightsTask` / `ThinExtractHighlightsPostProcess` | Babylon Lite<br>`createExtractHighlightsPostProcessTask` |
| Babylon.js<br>`FrameGraphChromaticAberrationTask` / `ThinChromaticAberrationPostProcess` | Babylon Lite<br>`createChromaticAberrationPostProcessTask` |
| Babylon.js<br>`FrameGraphBloomTask` / `BloomEffect` | Babylon Lite<br>`createBloomPostProcessTask` |
