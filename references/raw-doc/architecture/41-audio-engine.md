---
title: Audio Engine
source: https://doc.babylonjs.com/lite/architecture/41-audio-engine/
...
[Module: Audio Engine](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#module-audio-engine) [Purpose](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#purpose) [Design Mapping: AudioV2 → Lite](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#design-mapping-audiov2--lite) [Why collapse the abstract layer](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#why-collapse-the-abstract-layer) [Public API Surface](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#public-api-surface) [As-Built API & Divergences](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#as-built-api--divergences) [Sound sub-graph (signal flow)](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#sound-sub-graph-signal-flow) [Parameter ramp component](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#parameter-ramp-component) [Instances](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#instances) [State Machine / Lifecycle](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#state-machine--lifecycle) [Engine](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#engine) [Sound](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#sound) [Babylon.js Equivalence Map](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#babylonjs-equivalence-map) [Dependencies](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#dependencies) [Tree-Shaking / Bundle Strategy](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#tree-shaking--bundle-strategy) [Test Specification](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#test-specification) [Tier 1 — Unit / behavioral (mocked Web Audio API), deterministic — always-on CI gate](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#tier-1--unit--behavioral-mocked-web-audio-api-deterministic--always-on-ci-gate) [Tier 2 — Output correctness (`OfflineAudioContext` → PCM), opt-in (native) ★ primary correctness tier](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#tier-2--output-correctness-offlineaudiocontext--pcm-opt-in-native--primary-correctness-tier) [Tier 3 — Visual parity (offline PCM → waveform canvas), opt-in (native)](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#tier-3--visual-parity-offline-pcm--waveform-canvas-opt-in-native) [Tier 4 — Live demo visualizer, NOT a gate](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#tier-4--live-demo-visualizer-not-a-gate) [Behavioral parity against Babylon.js (optional, deterministic)](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#behavioral-parity-against-babylonjs-optional-deterministic) [File Manifest](https://doc.babylonjs.com/lite/architecture/41-audio-engine/#file-manifest)
...
| --- | --- |
| AudioV2 (Babylon.js)<br>`abstract AudioEngineV2` \+ `_WebAudioEngine` | Babylon Lite<br>one `AudioEngine` state interface + functions |
| AudioV2 (Babylon.js)<br>`abstract AbstractSound` \+ `_WebAudioStaticSound` | Babylon Lite<br>one `StaticSound` state interface + functions |
| AudioV2 (Babylon.js)<br>29 abstract classes + 21 `_WebAudio*` concretes | Babylon Lite<br>flat set of pure-state interfaces; no inheritance |
| AudioV2 (Babylon.js)<br>Class methods (`sound.play()`) | Babylon Lite<br>standalone functions (`playSound(sound)`) |
| AudioV2 (Babylon.js)<br>`new Observable()` event fields | Babylon Lite<br>lightweight callback-set (`AudioSignal<T>`) on the state |
| AudioV2 (Babylon.js)<br>Module-level `Instances[]`, `OnCreatedObservable` | Babylon Lite<br> **removed** — caller owns the `AudioEngine` handle |
| AudioV2 (Babylon.js)<br>Module-level `new RegExp`, cached `ExpCurve` | Babylon Lite<br>lazy-init inside functions (no module side effects) |
| AudioV2 (Babylon.js)<br>`WebRequest.FetchAsync` \+ custom headers | Babylon Lite<br>plain `fetch()` (matches every other Lite loader) |
| AudioV2 (Babylon.js)<br>`Vector3`/`Quaternion` from core Maths | Babylon Lite<br>Lite `math/` vec3/quat |
| AudioV2 (Babylon.js)<br>`Logger.Warn` | Babylon Lite<br>`console.warn` |
| AudioV2 (Babylon.js)<br>`EngineStore.LastCreatedEngine` (unmute UI) | Babylon Lite<br>explicit `parentElement` option |
| AudioV2 (Babylon.js)<br>`PrecisionDate.Now` | Babylon Lite<br>`performance.now()` |
| AudioV2 (Babylon.js)<br>`Node` / `AbstractMesh` (spatial attach) | Babylon Lite<br>Lite `Mesh` / `{ worldMatrix }` transform |
...
by its own function on a host (`AudioGraphHost = StaticSound | StreamingSound | AudioBus | AudioInputSource`), which lazily builds/rebuilds the sub-graph:
...
| --- | --- |
| Lite function/type<br>`createAudioEngineAsync` | AudioV2 origin<br>`CreateAudioEngineAsync` \+ `_WebAudioEngine` ctor/init |
| Lite function/type<br>`AudioEngine.state` | AudioV2 origin<br>`AudioEngineV2.state` (maps `ctx.state`) |
| Lite function/type<br>`unlockAudioEngineAsync` | AudioV2 origin<br>`_WebAudioEngine.unlockAsync` / `resumeAsync` |
| Lite function/type<br>`createSoundAsync` | AudioV2 origin<br>`CreateSoundAsync` \+ `_WebAudioStaticSound._initAsync` |
| Lite function/type<br>`createSoundBufferAsync` | AudioV2 origin<br>`CreateSoundBufferAsync` (`decodeAudioData`) |
| Lite function/type<br>`playSound`/`stopSound`/… | AudioV2 origin<br>`AbstractSound.play/stop/pause/resume` |
| Lite function/type<br>`SoundSubGraph` \+ rebuild | AudioV2 origin<br>`_WebAudioBusAndSoundSubGraph` \+ `_onSubNodesChanged` |
| Lite function/type<br>`applyRamp` / `getRampCurve` | AudioV2 origin<br>`_WebAudioParameterComponent` \+ `_GetAudioParamCurveValues` |
| Lite function/type<br>`SpatialSubNode` / panner | AudioV2 origin<br>`_SpatialWebAudioSubNode` (PannerNode config) |
| Lite function/type<br>`SpatialListener` | AudioV2 origin<br>`_SpatialWebAudioListener(+Fallback)` |
| Lite function/type<br>`attachSpatialTarget` | AudioV2 origin<br>`_SpatialAudioAttacherComponent.attach` |
| Lite function/type<br>`updateSpatialAudio` / RAF | AudioV2 origin<br>`_SpatialWebAudioUpdaterComponent` |
| Lite function/type<br>`createStreamingSoundAsync` | AudioV2 origin<br>`CreateStreamingSoundAsync` \+ `_WebAudioStreamingSound` |
| Lite function/type<br>`createMicrophoneSoundSourceAsync` | AudioV2 origin<br>`_WebAudioSoundSource` (getUserMedia) |
| Lite function/type<br>`createUnmuteUi` | AudioV2 origin<br>`_WebAudioUnmuteUI` (parentElement injected, no EngineStore) |
| Lite function/type<br>`createAudioEngineMediaStream` | AudioV2 origin<br>Lite-only `MediaStreamAudioDestinationNode` post-master tap |
